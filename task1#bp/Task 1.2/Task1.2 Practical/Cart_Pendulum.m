...
##*  Team Member Name: Amiya Ghosh,Dipanjan Maity,Anick Bhattacharya
##*
##*  
...
function dy = cart_pendulum_dynamics(y, m, M, L, g,  u)
  sin_theta = sin(y(3));#Sy
  cos_theta = cos(y(3));#Cy
  theta_dot = y(4);
  x_dot = y(2); 
  dy(1,1) = y(2) ;
  dy(2,1) = (m*sin_theta*(g*cos_theta-L*theta_dot^2))/(M+m*sin_theta^2) + (1/(M+m*sin_theta^2))*u;
  dy(3,1) = y(4);
  dy(4,1) = (-1)*(((M+m)*g-m*L*cos_theta*theta_dot^2)*sin_theta/(L*(M+m*sin_theta^2)))+(cos_theta/L*(M+m*sin_theta^2))*u;
endfunction
...
function [A, B] = cart_pendulum_AB_matrix(m , M, L, g)
  A = [0,1,0,0;0,0,-g/M,0;0,0,0,1;0,0,g*(1+M)/(M*L),0];
  B = [0;1/(m*M);0;-1/(L*m*M)];  
endfunction
